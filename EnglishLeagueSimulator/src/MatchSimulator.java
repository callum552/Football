...
public class MatchSimulator {
    ...
    /**
     * NEW METHOD: Simulates a match and returns the updated Match object with scores.
    ...
    public void simulateSingleMatch(Team homeTeam, Team awayTeam, boolean updateStats) {
        double homeLambda = calculateLambda(homeTeam, awayTeam, 1.25);
        double awayLambda = calculateLambda(awayTeam, awayTeam, 1.0);
        int homeGoals = getPoisson(homeLambda);
        int awayGoals = getPoisson(awayLambda);
        ...
        updateEloRatings(homeTeam, awayTeam, homeGoals, awayGoals);
    }
    ...
    protected Team resolveWithPenalties(Team team1, Team team2) {
        ...
        System.out.printf(" (%s wins %d-%d on penalties)\n", (team1Pens > team2Pens ? team1.name : team2.name), team1Pens, team2Pens);
    ...
    protected double calculateLambda(Team attackingTeam, Team defendingTeam, double advantageScaler) {
        ...
        double eloFactor = 1 + (eloDifference / 400.0) * 0.4;
    ...
    protected void updateEloRatings(Team team1, Team team2, int goals1, int goals2) {
        double result1 = 0.5;
        if (goals1 > goals2) result1 = 1.0;
        if (goals1 < goals2) result1 = 0.0;
        double expected1 = 1.0 / (1.0 + Math.pow(10, (team2.getEloRating() - team1.getEloRating()) / 400.0));
        team1.setEloRating(team1.getEloRating() + 50 * (result1 - expected1));
        team2.setEloRating(team2.getEloRating() + 50 * ((1 - result1) - (1 - expected1)));
    }
}